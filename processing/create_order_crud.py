...
def create_order_product(db: Session, order_prod: schemas.OrderProductsCreate):
    try:
        ...
        prods = db.query(models.Products).filter(models.Products.id==product_id).all()
        for prod_data in prods:
            ...
            order_db = db.query(models.Orders).filter(models.Orders.order_number == order_num).options(joinedload(models.Orders.orders_products)).all()

            new_total_amount = sum([op.total_amount for op in order_db])
            new_total_weight = sum([op.total_weight for op in order_db])

            # db.execute(
            #     update(models.Orders)
            #     .where(models.Orders.order_number == order_num)
            #     .values(total_amount=new_total_amount, total_weight=new_total_weight)
            # )
